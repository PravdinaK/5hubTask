...
public class LoggingAspect {
    ...
    @Before("execution(* com.fivehub.employee_service.controller.*.*(..))")
    public void logBefore(JoinPoint joinPoint) {
        log.info("Calling method: {} with parameters: {} ", joinPoint.getSignature().getName(), joinPoint.getArgs());
    }

    @AfterReturning(pointcut = "execution(* com.fivehub.employee_service.controller.*.*(..))", returning = "result")
    public void logAfter(JoinPoint joinPoint, Object result) {
        log.info("Method {} completed successfully and returning result: {} ", joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.fivehub.employee_service.controller.*.*(..))", throwing = "exception")
